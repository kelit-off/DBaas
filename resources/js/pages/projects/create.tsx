...
export default function CreateProject() {
    const { teams, team_id } = usePage().props;
    ...
    const submitForm = async (e) => {
        ...
        if (response.status === 201) {
            window.location.href = "/dashboard/project/"+response.data.project.slug
        }
    }
                    ...
                    <div className="space-y-6 px-6 py-6">
                        ...
                        <div className="grid grid-cols-3 items-center gap-4">
                            ...
                            <div className="col-span-2">
                                <Select name='team' defaultValue={formData.team} onValueChange={value => updateField("team", value)}>
                                    <SelectTrigger>
                                    ...
                                    <SelectContent>
                                        {teams.map((team: any) => (
                                            <SelectItem key={team.slug} value={team.slug}>
                                                {team.name} <span className="ml-2 text-xs text-neutral-400">{team.pricing_plan.name.charAt(0).toUpperCase() + team.pricing_plan.name.slice(1)} Plan</span>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                        ...
                        <div className="grid grid-cols-3 items-start gap-4">
                            ...
                            <div className="col-span-2 space-y-2">
                                <Input placeholder="Type in a strong password" name='password' onChange={e => updateField(e.target.name, e.target.value)}/>
                        ...
                        <div className="grid grid-cols-3 items-center gap-4">
                            <Label>Region</Label>
                            <Select defaultValue={formData.region} name='region' onValueChange={value => updateField("region", value)}>
